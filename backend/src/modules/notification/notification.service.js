            ...
            if (daysWithoutCleaning > 3) {
                if (!notifiedCageIds.has(Number(cage.id))) {
                    ...
                    await Notification.create({
                        ...
                        message: `La jaula #${cage.number} acumula ${daysWithoutCleaning} dÃ­as de ocupaciÃ³n sin limpieza. Â¡Por favor realiza la limpieza lo antes posible!`,
    ...
    async createRabbitAssignmentNotification(cageId, rabbitCode, isAssigned) {
        try {
            ...
            for (const wc of workerCages) {
                ...
                const title = isAssigned ? 'Nueva AsignaciÃ³n de Conejo' : 'Conejo Removido';
                const message = isAssigned 
                    ? `El conejo con cÃ³digo ${rabbitCode} ha sido asignado a tu jaula #${cage.number}.`
                    : `El conejo con cÃ³digo ${rabbitCode} ha sido removido de tu jaula #${cage.number}.`;
