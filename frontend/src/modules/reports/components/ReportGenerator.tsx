...
const formatDateTime = (dateString: string | null | undefined, includeTime = true): string => {
...
const capitalizeString = (str: string | null | undefined): string => {
    if (!str) return 'N/A';
    return str.charAt(0).toUpperCase() + str.slice(1);
};

const formatKitsStr = (bornKits: number | null | undefined, status: string): string => {
    if (bornKits === null || bornKits === undefined) return '-';
    const st = status.toLowerCase();
    if (['lactancia', 'completado'].includes(st)) {
        const suffix = bornKits === 1 ? 'vivo' : 'vivos';
        return `${bornKits} ${suffix}`;
    }
    if (['fallido', 'parcial'].includes(st)) {
        const suffix = bornKits === 1 ? 'muerto' : 'muertos';
        return `${bornKits} ${suffix}`;
    }
    return String(bornKits);
};

const buildReportSubtitle = (galpon: any, startDate: string, endDate: string): string => {
    const name = galpon?.name || 'Desconocido';
    const location = galpon?.location || 'Desconocido';
    const province = galpon?.province || 'Desconocido';
    return `Galpón: ${name}\nUbicación: ${location}, ${province}\nFecha del Reporte: Desde: ${startDate} Hasta: ${endDate}`;
};

const getDateFieldForModule = (item: any, module: string): string | null => {
    switch (module) {
...
const formatItemForPDF = (item: any, module: string): any => {
    const row = { ...item };
    switch (module) {
        case 'feeding': {
        ...
        case 'reproduction': {
            ...
            row.responsible = getResponsible(item);
            row.bornKitsStr = formatKitsStr(item.bornKits, item.status || '');
...
// ─── Static column definitions (no React state dependency) ───────────────────

const FEEDING_COLUMNS: { ui: any[]; pdf: PDFColumn[] } = {
    ui: [
            ...
            key: 'rabbits', header: 'Conejos', render: (row: any) => (
                <div className="flex flex-col gap-1">
                    {Array.isArray(row.rabbits) && row.rabbits.length > 0
                        ? row.rabbits.map((r: any) => <span key={r.code} className="whitespace-nowrap">{formatRabbitInfo(r.code, r.name)}</span>)
                        : 'Ninguno'
                    }
                </div>
            )
...
const VACCINATION_COLUMNS: { ui: any[]; pdf: PDFColumn[] } = {
    ui: [
        ...
        { key: 'cageNumber', header: 'Jaula', render: (row: any) => row.cageNumber || 'N/A' },
        {
            key: 'rabbitInfo', header: 'Conejo', render: (row: any) => (
                <span className="font-medium whitespace-nowrap">
                    {formatRabbitInfo(row.rabbitCode || row.rabbit?.code, row.rabbitName || row.rabbit?.name)}
                </span>
            )
        },
...
const DEWORMING_COLUMNS: { ui: any[]; pdf: PDFColumn[] } = {
    ui: [
...
const CLEANING_COLUMNS: { ui: any[]; pdf: PDFColumn[] } = {
    ui: [
        { key: 'cleaningDate', header: 'Fecha y Hora', render: (row: any) => formatDateTime(row.cleaningDate) },
        {
            key: 'cageNumber', header: 'Jaula',
            render: (row: any) => formatCageNumber(row.cageNumber || row.Cage?.number, row.cageType || row.Cage?.type)
        },
        {
            key: 'rabbits', header: 'Conejos', render: (row: any) => (
                <div className="flex flex-col gap-1">
...
const MORTALITY_COLUMNS: { ui: any[]; pdf: PDFColumn[] } = {
    ui: [
...
const buildReproductionColumns = (selectedStatuses: string[]): { ui: any[]; pdf: PDFColumn[] } => {
    ...
    const uiColumns: any[] = [
        ...
        { key: 'status', header: 'Estado', render: (row: any) => <span className="capitalize">{row.status}</span> },
        { key: 'responsible', header: 'Reportado por', render: (row: any) => getResponsible(row) }
    ];
    ...
    const pdfColumns: PDFColumn[] = [
        ...
        { header: 'Estado', dataKey: 'formattedStatus' },
        { header: 'Reportado por', dataKey: 'responsible' }
    ];
    ...
    if (showCausa) {
        uiColumns.push({
            key: 'cause',
            header: 'Causa',
            render: (row: any) => row.cancellationReason ? capitalizeString(row.cancellationReason) : '-'
        });
...
const MODULE_COLUMNS: Record<string, { ui: any[]; pdf: PDFColumn[] }> = {
    feeding: FEEDING_COLUMNS,
    vaccination: VACCINATION_COLUMNS,
    deworming: DEWORMING_COLUMNS,
    cleaning: CLEANING_COLUMNS,
    mortality: MORTALITY_COLUMNS,
};
...
export function ReportGenerator({ galponId, galpon, onToast }: Readonly<ReportGeneratorProps>) {
    ...
    // getColumns complexity: 2 (function + 1 branch) — well under the 15 limit
    const getColumns = (): { ui: any[]; pdf: PDFColumn[] } => {
        if (selectedModule === 'reproduction') {
            return buildReproductionColumns(selectedStatuses);
        }
        return MODULE_COLUMNS[selectedModule] ?? { ui: [], pdf: [] };
    };
    ...
    // getDateFieldForModule moved to module level to reduce nesting depth
    const sortedPreviewData = useMemo(() => {
        ...
        if (sortBy === 'date') {
            data.sort((a, b) => {
                const dA = getDateFieldForModule(a, selectedModule);
                const dB = getDateFieldForModule(b, selectedModule);
                if (!dA || !dB) return 0;
    ...
    // formatItemForPDF and buildReportSubtitle are module-level — reduces buildPdf complexity from 26 to ~4
    useEffect(() => {
        ...
        const buildPdf = async () => {
            ...
            const subtitle = buildReportSubtitle(galpon, appliedStartDate, appliedEndDate);
            const formattedData = sortedPreviewData.map(item => formatItemForPDF(item, selectedModule));
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            a.download = `${galponName}_${reportName}_${year}-${month}-${day}.pdf`;
